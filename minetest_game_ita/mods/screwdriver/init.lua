...
local mode_text = {
	{"Cambiare rotazione, non cambiare la direzione dell'asse."},
	{"Mantenere davanti la faccia scelta e poi ruotarla."},
	{"Cambiare la direzione dell'asse, azzerare la rotazione."},
	{"Portare in fronte la parte alta e poi ruotarla."},
}
...
local function screwdriver_setmode(user,itemstack)
	...
	if not mode then
		minetest.chat_send_player(player_name, "Tenere premuti 'maiusc' e 'usa' per cambiare la modalità del cacciavite.")
		mode = 0
	end
	mode = mode + 1
	if mode == 5 then
	...
	minetest.chat_send_player(player_name, "Modalità del cacciavite: "..mode.." - "..mode_text[mode][1] )
...
local function get_node_face(pointed_thing)
	...
	elseif ay < uy then return 5 -- Bottom
		...
		error("pointed_thing.above sopra e sotto sono uguali!")
	end
end
...
for i = 1, 4 do
	minetest.register_tool("screwdriver:screwdriver"..i, {
		description = "Cacciavite in modalità "..i,
